...
async def newsk(message: types.Message):
    b = str(message.from_user.id)
    a = str(requests.get('http://127.0.0.1:80/newsk/' + b).text)
    f = eval(a)
    for i in range(len(f)):
